...
def loop(site,suf):
    ...
    mergeresult(Location, ALOS_suf, result_suf)
...
def zonalstats(Location,ALOS_suf,result_suf):
    print("Zonal stats: ALOS polygon and CHM as inputs")
...
def join(Location,result_suf):
    ...
    inpolygon = ArcGIS_dir + Location + result_suf + "_i"
    arcpy.JoinField_management(inpolygon, "id", inpolygon + "_z_1", "id", ["SUM"])
...
def mergeresult(Location,ALOS_suf,result_suf):
    target_features = ArcGIS_dir + Location + result_suf + "_i_w"
    inpolygon = ArcGIS_dir + Location + result_suf + "_p"
    out_feature_class = ArcGIS_dir + Location + result_suf + "_result_cover"
...
def output(Location,result_suf):
    inpolygon = ArcGIS_dir + Location + result_suf + "_result_cover"
    ...
    arcpy.Rename_management(DB_shp + 'lyr.shp', DB_shp + Location + result_suf + "_result_cover.shp")
...
def spatialjoin(Location,result_suf):
    ...
    target_features = ArcGIS_dir + Location + result_suf + "_i_w"
    out_feature_class3 = ArcGIS_dir + Location + result_suf + "_result_cover"
...
def change_cover():
    arcpy.env.workspace = DB_shp
    fc_list = arcpy.ListFeatureClasses()
    for fc in fc_list:
        if "cover" not in fc and "HV" not in fc and fc.split("_")[3][0] != "H":
            cover1 = fc.split("_")[0] + "_ALOS_" + fc.split("_")[2] + "_" + fc.split("_")[4] + "_" + fc.split("_")[5] + \
                     "_result_cover.shp"
            cover2 = fc.split("_")[0] + "_ALOS_" + fc.split("_")[3] + "_" + fc.split("_")[4] + "_" + fc.split("_")[5] + \
                     "_result_cover.shp"
            print(cover1)
            print(cover2)
            xls = DB_dir + fc.split(".")[0] + ".xlsx"
            print("Output xlsx: " + xls)
            xls1 = DB_dir + cover1.split(".")[0] + ".xlsx"
            print("Output xlsx: " + xls1)
            xls2 = DB_dir + cover2.split(".")[0] + ".xlsx"
            print("Output xlsx: " + xls2)
            arcpy.TableToExcel_conversion(fc, xls)
            arcpy.TableToExcel_conversion(cover1, xls1)
            arcpy.TableToExcel_conversion(cover2, xls2)
            fields = ['pointid', 'CA_o_mean3','CA_o_mean5','CA_o_mean9']
            fields1 = ['pointid', 'C_c_sum3', 'C_c_sum5', 'C_c_sum9']
            data_xls = pd.read_excel(xls, header=0,usecols=fields)
            data_xls1 = pd.read_excel(xls1, header=0,usecols=fields1)
            data_xls2 = pd.read_excel(xls2, header=0,usecols=fields1)
            print("Merging...")
            df_merge = pd.merge(data_xls, data_xls1, left_on='pointid', right_on='pointid', how='left')
            df_merge1 = pd.merge(df_merge, data_xls2, left_on='pointid', right_on='pointid', how='left')
            #print("Calculating cover...")
            df_merge1["C_c_sum3"] = df_merge1['C_c_sum3_x'] - df_merge1['C_c_sum3_y']
            df_merge1["C_c_sum5"] = df_merge1['C_c_sum5_x'] - df_merge1['C_c_sum5_y']
            df_merge1["C_c_sum9"] = df_merge1['C_c_sum9_x'] - df_merge1['C_c_sum9_y']
            out = DB_csv + fc.split(".")[0] + "_cover.csv"
            print("Output csv..." + out)
            df_merge1.to_csv(out, index=None)
