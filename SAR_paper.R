...
#filter out 1.5m for volume/height
out <- "E:\\ChangMap\\CHM\\DB_20220221\\DB_chm\\Noshurb"
Locdir <- "E:\\ChangMap\\CHM\\DB_20220221\\DB_chm\\Mosaic"
chms <- list.files(path = Locdir, full.names = TRUE, pattern = "*.tif$")
for (chm in chms){
  print(chm)
  r <- raster(chm)
  x <- clamp(r, lower=1.5, useValues=FALSE)
  writeRaster(x,file.path(out,paste0(file_path_sans_ext(basename(chm)),".tif")),options=c('TFW=YES'),overwrite=TRUE)
}
...
p1<- ggplot(Data1, aes(x=x1, y=y1))+ 
  ...
  geom_smooth(
    ...
    linetype = "solid",
    colour = "blue")+
  annotate("text", x=0.55, y=-20, hjust = 0,color="black",size = 8,
           label= paste("\n",expression(WCM),": ",round(R2_1,3),"\n",
                        expression(Linear),": ",round(summary(reg1_1)$adj.r.squared,3),"\n",
                        expression(Log),": ",round(summary(reg3_1)$adj.r.squared,3)
           )) + 
  ...
  theme(text=element_text(size=25)) + 
  theme(plot.title = element_text(hjust = 0.5))+
  theme(legend.position=c(0.75,0.12),legend.key.width=unit(3,"cm"))
...
p2<- ggplot(Data2, aes(x=x2, y=y2))+ 
  ...
  geom_smooth(
    ...
    linetype = "solid",
    colour = "blue")+
  annotate("text", x=0.5, y=-20, hjust = 0,color="black",size = 8,
           label= paste("\n",expression(WCM),": ",round(R2_2,3),"\n",
                        expression(Linear),": ",round(summary(reg1_2)$adj.r.squared,3),"\n",
                        expression(Log),": ",round(summary(reg3_2)$adj.r.squared,3)
           )) + 
...
# ------------------------------------------------------------------------------------------------ #
#bias boxplot 
dir1 <- paste0("F:\\DB_csv\\Merge_2018.csv")
dir2 <- paste0("F:\\DB_csv\\Merge_2008.csv")
...
dir1 <- paste0("F:\\DB_csv\\Merge_2018.csv")
dir2 <- paste0("F:\\DB_csv\\Merge_2010.csv")
...
dir1 <- paste0("F:\\DB_csv\\Merge_2010.csv")
dir2 <- paste0("F:\\DB_csv\\Merge_2008.csv")
...
#group
Data$Type[Data$diff_C2_ALS_5 >= 0.6] = ">0.6"
Data$Type[Data$diff_C2_ALS_5 >= 0.4 & Data$diff_C2_ALS_5 < 0.6] = "0.4~0.6"
Data$Type[Data$diff_C2_ALS_5 >= 0.2 & Data$diff_C2_ALS_5 < 0.4] = "0.2~0.4"
Data$Type[Data$diff_C2_ALS_5 >= 0 & Data$diff_C2_ALS_5 < 0.2] = "0~0.2"
Data$Type[Data$diff_C2_ALS_5 >= -0.2 & Data$diff_C2_ALS_5 < 0] = "-0.2~0"
Data$Type[Data$diff_C2_ALS_5 >= -0.4 & Data$diff_C2_ALS_5 < -0.2] = "-0.4~-0.2"
Data$Type[Data$diff_C2_ALS_5 >= -0.6 & Data$diff_C2_ALS_5 < -0.4] = "-0.6~-0.4"
Data$Type[Data$diff_C2_ALS_5 <= -0.6] = "-0.6<"
Data$Type<-factor(Data$Type, levels=c("-0.6<","-0.6~-0.4", "-0.4~-0.2","-0.2~0",
...
Data <- Data %>%
  group_by(Type) %>%
  do(sample_n(., 300, replace = TRUE))

#two-way ANOVA + Tukey + se
test_anova <- aov(diff_C2_SAR_5 ~ Type, data = Data)
TukeyHSD(test_anova)
mean_se <- Data %>%
  group_by(Type) %>%
  summarise(Means = mean(diff_C2_SAR_5), SEs = sd(diff_C2_SAR_5)/sqrt(n()))
...
Data$Type[Data$diff_C2_ALS_5 >= 0.0 & Data$diff_C2_ALS_5 < 0.25] = "0~0.25"
Data$Type[Data$diff_C2_ALS_5 >= -0.25 & Data$diff_C2_ALS_5 < 0] = "-0.25~0"
Data$Type[Data$diff_C2_ALS_5 >= -0.5 & Data$diff_C2_ALS_5 < -0.25] = "-0.5~-0.25"
Data$Type[Data$diff_C2_ALS_5 <= -0.5] = "-0.5<"
Data$Type<-factor(Data$Type, levels=c("-0.5<","-0.5~-0.25", "-0.25~0",
                                      "0~0.25","0.25~0.5",">0.5"))
